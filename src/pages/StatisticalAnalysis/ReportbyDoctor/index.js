...
import { FinancialmanagementApi, userApi } from '@/services/basic';
...
class ReportbyDoctor extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  docterSearch = (yhxm) => {
    ...
    return userApi.getPage({ formData: { yhbs: 1, yhxm, current: page } }).then((res) => {
      const data = res.records.map((it) => (
        <Select.Option key={it.id} value={it.id}>
          {it.yhxm}
        </Select.Option>
      ));
      const initys = res.records && res.records.length > 0 ? res.records[0].id : null;
      this.setState({ ysData: data, page: res.records.length < 7 ? 1 : res.pages }, () => {
        this.form.setFieldsValue({ yhid: initys });
      });
    });
  };
  //查找框查找数据
  queryData = () => {
    this.form.validateFields().then((values) => {
      ...
      this.setState({ records: [], loading: true, charts: false, option: {}, ysxm: null }, () => {
        FinancialmanagementApi.docter({
  ...
  render() {
    const { records, option, charts, loading, ysData, initys } = this.state;
        ...
        <Form autoComplete='off' ref={(form) => (this.form = form)} layout='inline'>
          <InputGroup className='input-group'>
            <Row gutter={24}>
              ...
              {yhbs === 3 ? (
                <Col span={6}>
                  <Form.Item label='医生姓名' name='yhid' rules={[{ required: true, message: '此为必填项' }]} initialValue={initys}>
                    <Select
                      showSearch
                      placeholder='请选择医生'
                      ...
                      showArrow={false}
                      filterOption={false}
                      onSearch={this.docterSearch}
                      notFoundContent={null}
                      dropdownRender={(menu) => (
                        <div>
                          {menu}
                          <Divider style={{ margin: '4px 0' }} />
                          <Button block icon={<SyncOutlined />} onClick={() => this.docterSearch()}>
                            换一批
                          </Button>
                        </div>
                      )}
              ...
              <Col span={2}>
                <TableExport DCdata={records} name='isme' filename='互联网问诊统计表 --- 按医生'></TableExport>
              </Col>
            </Row>
          </InputGroup>
        </Form>
...
export default BtnPermission(ReportbyDoctor);
